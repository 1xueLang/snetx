...
__all__ = ["Compose", "ToPILImage", "ToTensor", 'GestureRoll', 'SkipFrames']
...
class GestureRoll(object):
    def __init__(self, T):
        self.T = T
    
    def __call__(self, x):
        x = torch.Tensor(x)
        shifts = random.randint(0, self.T * 2)
        if shifts < self.T:
            x = torch.roll(x, self.T, 0)
        return x
